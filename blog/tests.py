...
class TestView(TestCase):
    ...
    def test_create_post(self):
        ...
        self.client.post(
            '/blog/create_post/',
            {
                ...
                'tags_str': 'new tag; good tag', 'python'
            }
        )
        ...
        self.assertTrue(Tag.objects.get(name='good tag'))
    ...
    def test_comment_form(self):
        ...
        response = self.client.post(self.post_001.get_absolute_url() + 'new_comment/',
                    {
                        'content': "This is comment from obama."
                    },
                    follow = True
                )
        self.assertEqual(response.status_code, 200)
        ...
        self.assertIn('This is comment from obama.', new_comment_div.text)

    def test_comment_update(self):
        comment_by_trump = Comment.objects.create(
            post = self.post_001,
            author = self.user_trump,
            content = 'This is comment from trump'
        )
        response = self.client.get(self.post_001.get_absolute_url())
        self.assertEqual(response.status_code, 200)
        soup = BeautifulSoup(response.content, 'html.parser')
        ...
        response = self.client.post(
            ...
            {
                'content': "update obama`s comment"
            },
            follow = True
        )
        ...
        self.assertIn('update obama`s comment', comment_001_div.text)
    ...
    def test_delete_comment(self):
        ...
        self.client.login(username='trump', password='sompassword')
        response = self.client.get(self.post_001.get_absolute_ulr())
        self.assertEqual(response.status_code, 200)
        soup = BeautifulSoup(response.content, 'html.parser')
        ...
        self.assertIn(self.post_001.title, soup.title.text)
        comment_area = soup.find('div', id='comment-area')
        self.assertNotIn('This is comment from trump', comment_area.text)
    ...
    def test_search(self):
        post_about_python = Post.objects.create(
            title='This is post about python',
            content='Hello World',
            author=self.user_trump
        )

        response = self.clinet.get('/blog/search/python/')
